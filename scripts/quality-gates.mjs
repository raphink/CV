...
gate('P7 mobile selection reveals evidence immediately', all(js, ["matchMedia('(max-width: 900px)')", 'scrollToMobileRoute', "panel.focus({ preventScroll: true })"]));
...
gate('P11 desktop routes advance in one scroll-driven stage', all(html, ['class="route-explorer"', 'class="route-stage"', 'data-scroll-route="devops"', 'data-scroll-route="platform"', 'data-scroll-route="education"']) && all(js, ['prepareRouteNarrative', "rootMargin: '-47% 0px -47% 0px'", 'scrollToRoute']) && all(mobileCss, ['.route-stage {', 'position: sticky', '.route-scroll-stop', 'height: max(72vh, 590px)', '.route-scroll-track {', 'display: none']));
gate('P12 mobile routes read as complete sequential chapters', html.includes('id="mobile-route-sequence"') && all(js, ['buildMobileRouteSequence', 'routeChapterLabels', 'data-mobile-route=', "classList.add('route-sequence-ready')", "rootMargin: '-30% 0px -60% 0px'", "button.setAttribute('aria-controls', 'mobile-route-'", "routesControl.setAttribute('role', mobileRouteMedia.matches ? 'navigation' : 'tablist')"]) && all(mobileCss, ['.mobile-route-sequence', '.route-sequence-ready .mobile-route-sequence', 'display: grid', '.route-sequence-ready .route-stage > .route-detail', 'display: contents']));
